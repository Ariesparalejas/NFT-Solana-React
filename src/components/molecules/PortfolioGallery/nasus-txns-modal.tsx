...
const NasusTxnsModal: FC<Props> = ({ handleClose, show, walletAddr }) => {
  const [nasusTxns, setNasusTxns] = useState<Solana.NasusSolanaTransaction[]>();
  const router = useRouter();
  useEffect(() => {
    async function getAndSet() {
      try {
        const url = `https://ouo4ylg48g.execute-api.us-east-2.amazonaws.com/prod/nasus-txns/wallet/${walletAddr}`;
        const res = await fetch(url);
        const nt = await res.json();
        const rj =
          nt && (nt["nasusTxnsForWallet"] as Solana.NasusSolanaTransaction[]);
        if (rj) setNasusTxns(rj);
      } catch (error: any) {
  ...
  const handleCloseImpl = () => {
    logEvent({ category: "portfolio nasus txns modal", action: "dismiss" });
    handleClose();
  };
  ...
  if (nasusTxns && nasusTxns.length < 1) {
  }
        ...
        <Modal.Header closeIcon={true} onClosePress={handleCloseImpl}>
          <span>
            ...
            <div className="text-sm text-primary-dark">
              <strong>Found {nasusTxns.length}</strong> NFT trading events
